...
@Component
@AllArgsConstructor
public class PlaybackActionNextStrategy implements PlaybackActionStrategy {

    @Override
    public void execute(SpotifyApi spotifyApi, ClientDeviceEntity clientDevice) {
        try {
            spotifyApi.skipUsersPlaybackToNextTrack().device_id(clientDevice.getDeviceId()).build().execute();
