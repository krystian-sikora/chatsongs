...
@Component
@AllArgsConstructor
public class PlaybackActionPreviousStrategy implements PlaybackActionStrategy {

    @Override
    public void execute(SpotifyApi spotifyApi, ClientDeviceEntity clientDevice) {
        try {
            spotifyApi.skipUsersPlaybackToPreviousTrack().device_id(clientDevice.getDeviceId()).build().execute();
    ...
    public PlaybackActions getAction() {
        return PlaybackActions.PREVIOUS;
    }
}
